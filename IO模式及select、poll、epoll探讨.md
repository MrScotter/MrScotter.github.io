# <center> IO模式及select、poll、epoll探讨</center>
...
四 IO多路复用与多进程/多线程之间的选择
IO多路复用、多进程/多线程都可以实现对多路IO进行有效监控，那在生产环境中，我们应该选择使用哪种方式呢?选择首要考虑的因素因为是业务消耗资源的类型：CPU密集型/IO密集型。再做选择时，我们来分析两种方式的一些区别：

IO多路复用：没有进程线程创建的开销，但对于到来的业务，其处理方式的串行的。
多进程/多线程：可并行处理有个业务(SMP环境中)，但其创建过程需要消耗过多的CPU时间，并且每个task_struct都占用了较大的内存。
其后，我们针对不同的业务类型进行分析:
CPU密集型：消耗较多CPU资源，业务一般需要运行较长时间，task创建和销毁的开销所占比例小。适用于多进程、多线程。
IO密集型：消耗较少的CPU资源，大部分时间在等待IO数据准备好，单个task就能很好的服务多路IO；若使用多进程/多线程，task的创建和销毁时间，反而占据了其整个业务时间比重过大。适用于IO多路复用。
nginx和ligty实例分析：“为什么nginx/ligty用多进程+io多路复用就获得高效结果呢”